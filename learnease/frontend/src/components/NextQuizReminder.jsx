    ...
    fetch(`${API}/getall/${user.id}`)
      ...
      .then(data => {
        const sorted = data
          .map(item => ({
            ...item,
            fullDate: parseDateTime(item.quizDate, item.quizTime),
          }))
          .sort((a, b) => a.fullDate - b.fullDate);
        setUpcoming(sorted.slice(0, 3)); // Only 3 upcoming quizzes
      });
  }, [user]);

  // Countdown timers
// Countdown timers with days/hours/minutes/seconds
useEffect(() => {
  ...
  const interval = setInterval(() => {
    ...
    upcoming.forEach(item => {
      ...
      if (diff <= 0) updatedTimers[item._id] = "Started";
      else {
        let remaining = diff;
    ...
    <div className="mt-10 mx-5 p-6 bg-gray-900 border border-gray-700 rounded-2xl shadow-xl">
      ...
      <div className="grid md:grid-cols-3 gap-4">
        {upcoming.map(item => (
          <div
            key={item._id}
            className="p-5 bg-gray-800 rounded-xl border border-gray-700 shadow-lg hover:shadow-red-500/20 transition"
          >
            <h3 className="text-xl font-bold text-yellow-400">{item.quizName}</h3>
            <p className="text-gray-300 mt-2">
  📆 {formatReadableDate(item.quizDate)}
</p>

            <p className="text-gray-300">⏰ {format12Hour(item.quizTime)}</p>
...
export default NextQuizReminder;
